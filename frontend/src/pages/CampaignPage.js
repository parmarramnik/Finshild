...
const CampaignPage = () => {
  ...
  const [form, setForm] = useState({
    name: '', template_id: '', target_departments: [], status: 'draft'
  });
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ name: '', template_id: '', target_departments: [], status: 'draft' });
      fetchData();
    } catch (err) {
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="bg-white rounded-xl shadow overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {campaigns.length === 0 ? (
              <tr><td colSpan="5" className="px-4 py-8 text-center text-slate-400">No campaigns yet</td></tr>
            ) : (
              campaigns.map(c => (
